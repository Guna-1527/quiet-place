...
const Categories = () => {
  return (
    <div className="w-full h-[400px] flex flex-col p-12 items-center">
      <h1 className="text-xl font-bold text-[#7C4DFF]">
        Popular Quiz Categories
      </h1>
      <div className="flex flex-wrap justify-around max-w-5xl gap-10 mt-10">
        {category.map((item) => (
          <div className="flex items-center border-1 rounded-md border-gray-300 px-8 py-4">
            <div className="image w-[40px] h-[40px]">
              <Image src={item.image} alt="logo" />
            </div>
            <div className="font-bold">
              <h2 className="font-bold">{item.title}</h2>
              <p className="text-sm text-gray-300">{item.test}</p>
            </div>
          </div>
        ))}
      </div>
